...
pub fn draw_pose(
    ...
    draw: &Draw,
    pixels_per_m: f32,
    color: nannou::color::Rgba8,
) {
    // Circle for position
    draw.ellipse()
        .x_y(pixels_per_m * pose.x, pixels_per_m * pose.y)
        .radius(5.)
        .color(color);
    // Line for orientation
    let line_length = 15.;
    let start = pt2(pixels_per_m * pose.x, pixels_per_m * pose.y);
    let end = start
        + pt2(
...
pub fn draw_pose_graph(pose_graph: &PoseGraph, draw: &Draw, pixels_per_m: f32) {
    let visible_poses = 10;
    let step: u8 = 255 / visible_poses;
    let alpha_vals = (0..pose_graph.nodes.len())
        .into_iter()
        .rev()
        .map(|i| {
    ...
    for (node, alpha) in pose_graph.nodes.iter().zip(alpha_vals) {
        ...
        draw_pose(pose, draw, pixels_per_m, rgba(194, 61, 255, alpha));
    }
}

pub fn draw_scan_points(
    points: &Vec<Point2>,
    draw: &Draw,
    pixels_per_m: f32,
    color: nannou::color::Rgb8,
    origin: diff_drive::Pose,
) {
    ...
    for pt in points {
        ...
        draw.ellipse()
            .x_y(
                pixels_per_m * tf_pt[0] as f32,
                pixels_per_m * tf_pt[1] as f32,
            )
